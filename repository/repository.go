...
func (r *repository) GetArticle(ctx context.Context, id *string) (*domain.Article, error) {
	...
	query := `SELECT title, content, status, creator_id, created_at, updated_at FROM t_article WHERE id = $1`
	row := r.pg.QueryRowContext(ctx, query, id)

	if err := row.Scan(&res.Title, &res.Content, &res.Status, &res.CreatorID, &res.CreatedAt, &res.UpdatedAt); err != nil {
		return nil, err
	}
...
func (r *repository) AddArticle(ctx context.Context, id *string) (*domain.Article, error) {
	return nil, nil
}
